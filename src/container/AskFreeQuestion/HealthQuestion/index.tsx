...
const HealthQuestion = memo(({route}: any) => {
  ...
  const [dataCondition, setDataCondition] =
    useState<Array<dataAddition>>(DATA_ADDITION);
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <Content
        ...
        <Container style={styles.frame}>
          ...
          <ButtonLinear
            white={isCheckAddition}
            title={'Continue'}
            children={
              isCheckAddition && lengthText > 30 ? (
                <Image source={ICON.next} style={styles.buttonCheck} />
              ) : (
                <Image source={ICON.next} style={styles.buttonChildren} />
              )
            }
