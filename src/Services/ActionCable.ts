...
class ActionCableConnector extends BaseActionCableConnector {
  constructor(pubsubToken:string, webSocketUrl:string, accountId:string, userId:string) {
  ...
  onMessageCreated = (message) => {
    // store.dispatch(addMessageToConversation({ smessage }));
  };
...
export default {
  init({ pubSubToken, webSocketUrl, accountId, userId }:any) {
    const actionCablee = new ActionCableConnector(pubSubToken, webSocketUrl, accountId, userId);
