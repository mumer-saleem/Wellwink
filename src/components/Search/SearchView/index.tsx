...
const SearchView = memo(({onCancel, searchKey, ...props}: SearchViewProps) => {
  const {navigate} = useNavigation();
  const [_searchKey, setSearchKey] = useState<any>(searchKey ? searchKey : '');
  ...
  const onChangeSearchKey = useCallback(text => {
    ...
    for (let i = 0; i < DATA_SPECIALITIES.length; i++) {
      if (changeAlias(DATA_SPECIALITIES[i].name).includes(changeAlias(text))) {
    ...
    for (let i = 0; i < DATA_DOCTOR.length; i++) {
      if (changeAlias(DATA_DOCTOR[i].doctor.name).includes(changeAlias(text))) {
